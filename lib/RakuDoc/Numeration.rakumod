...
method numform (
    Str :$form is copy,                # The format string (contents of the :form<...>)
    Str :$contents = q{},              # The contents contained in the block
    Str :$type = q{},                  # The type of the block
    Str :$caption = q{}                # The caption to be shown
){
    # What a :form<> format consists of...
    grammar Format {
        token TOP          { ^ <verbatim>* %% <field> $ }
        token field        { \% [ <literal> | <fieldname> <options>* ] }
        token literal      { \% | \: }
    ...
    my @nums = @!counters;    # The hierarchical enumeration values of the block
    ...
    state %ord
        ...
        = :en( { $_ ~ ( /<!after 1> (<[1..3]>) $/ ?? ["","st","nd","rd"][$0] !! 'th') }),
          ...
          :cy( { my @ôl-ddodiaid is default('ain')
        ...
        = :zh({ $^numstr.trans('零壹貳叁肆伍陸柒捌玖拾佰仟' => '〇一二三四五六七八九十百千') }),
          :ja({ $^numstr.trans(
                     '零壱弐貳参參肆伍陸漆柒質捌玖拾佰仟萬' => '〇一二二三三四五六七七七八九十百千万',
                                                       '廿' => '二十',
                                                       '卅' => '三十',
                ).subst(/^ 一 <before <[十百千]>>/, '');  # Remove formal leading 1
             });
    ...
    state %uc
        ...
        = :zh({ $^numstr.trans('〇一二三四五六七八九十百千' => '零壹貳叁肆伍陸柒捌玖拾佰仟') }),
          :ja({ $^numstr.trans(
                ...
                ).subst(/^ <before <[拾佰仟]>>/, '壱');  # Add formal leading 1
             });
    ...
    sub make-sc-for ($uc-required) {
        sub ($str) {
            ...
            $str.subst: /\S+/, { LEAVE $new-sentence = ?/ <:SentenceTerminal> $ /;
                                   $_ ~~ $uc-required ?? .uc
                                !! $new-sentence      ?? .tclc
                                !!                       .lc   }, :g;
            }
    ...
    state %sc is default(make-sc-for({ !/<lower>/ }))
        = :en( make-sc-for({ !/<lower>/ || /^<[io]><-:Letter>*$/ }) );  # Also capitalize "I" and "O"
    ...
    # Track any warnings...
    my @warnings will leave { $^each».note };
    sub warn (*@msg) { @warnings.unshift: @msg.join(q{ }) }

    # Handle the various components a format may contain...
    multi handle (:$verbatim) { $verbatim }
    multi handle (:field($_)) {
        ...
        if %case > 1 || $implied-lc && %case == 1 && !%case<lc> {
            if $implied-lc && %case<uc tc tclc sc pc>.any {
                warn "Can't specify {':' «~« %case.keys} on field $raw-fieldname",
                     "(which implies :lc). Using :lc";
                %case = :lc;
            }
            else {
                warn "Can't specify {':' «~« %case.keys} on $raw-fieldname. Using :uc";
                %case = :uc;
            }
        }
        ...
        my $value = do given $fieldname {
            when 'C'            { $caption  // q{}                                                   }
            when 'D'            { $contents // q{}                                                   }
            when 'T'            { $type.subst(/^num/, '').tclc                                   }
            when 'A'            { my $n = @nums.pop; !$n ?? q{} !! ('', 'A' ... 'ZZZZZZZZZ')[$n] }
            when 'N'            { my $n = @nums.pop; !$n ?? q{} !! %num{$lang}($n)               }
            when $is-lang-field { my $n = @nums.pop; !$n ?? q{} !! %num{$lang}($n)               }
            default             { warn "Field $raw-fieldname is reserved for future use";
                                  $raw-fieldname;
                                }
        }
    ...
    return Format.parse($form).caps.reverse.map({ handle |$^cap }).reverse.join;
}
