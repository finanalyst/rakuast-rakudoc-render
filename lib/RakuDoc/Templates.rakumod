...
class Template {
    ...
    multi method AT-KEY(Str:D $key) { self.CALL-ME($key) }
...
class X::Unexpected-Template is Exception {
    has $.key;
    method message { "Template ｢$.key｣ is not known" }
}
...
class PStr {
    ...
    multi method pre( Str(Any) $s ) { @!string.unshift( $s ); self }
    multi method pre( PCell $s ) { @!string.unshift( $s ); self }
    multi method post( Str(Any) $s ) { @!string.push( $s ); self }
    multi method post( PCell $s ) { @!string.push( $s ); self }
    multi method merge( PStr $s ) { @!string.push( $s.string ); self }
    method lead() {
        my Str $rv;
...
multi sub infix:<~>( Str $s, PStr $p --> PStr) is export { $p.pre( $s ) }
multi sub infix:<~>( PStr $p, Str $s --> PStr ) is export { $p.post( $s ) }
multi sub infix:<~>( PCell $s, PStr $p --> PStr) is export { $p.pre( $s ) }
multi sub infix:<~>( PStr $p, PCell $s --> PStr ) is export { $p.post( $s ) }
multi sub infix:<~>( PStr $p, PStr $s --> PStr ) is export { $p.merge( $s ) }
